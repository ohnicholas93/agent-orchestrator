...
- Sleep via the local orchestrator ONLY, not harness timers. Treat this as the durable wakeup path for long waits instead of spawning shell `sleep` processes from the harness. NEVER use harness timers (exec_command with sleep) for sleeps; they are not reliable.
